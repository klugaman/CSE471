...
bool CSynthesizer::Generate(double * frame)
{
	...
	while (m_currentNote < (int)m_notes.size())
	{
		...
		shared_ptr<CInstrument> instrument = nullptr;
		if (note->Instrument() == L"ToneInstrument")
		{
			instrument = make_shared<CToneInstrument>();
		}
		...
		if (instrument != NULL)
		{
			...
			instrument->SetBPM(GetBeatsPerMinute());
	...
	for (list<shared_ptr<CInstrument>>::iterator node = m_instruments.begin(); node != m_instruments.end();)
	{
		...
		list<shared_ptr<CInstrument>>::iterator next = node;
		next++;

		// Get a pointer to the allocated instrument
		shared_ptr<CInstrument> instrument = *node;
