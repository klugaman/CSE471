...
public:
    ...
    /*! Sets the image size identical to that of another
        ...
        \param img The other image 
        \param planes Optional number of planes for new image */
...
\endcode
        ...
        \param i Row number
        \returns Constant pointer to the row */
...
\endcode
        ...
        \param i Row number
        \returns Pointer to the image row */
    ...
    /*! This function can be used to directly access the 
        ...
        word (32 bit) boundary. 
        \returns The image bits as an array */
    ...
    //! \brief Get the row pitch
    //! \return The row pitch. This is the number of bytes from
    //! the start of one row to the start of the next.
    int GetRowPitch() const;

    //! \brief Access any error message
    //! Error messages can be generated when a file is
    //! loaded. If the function returns false, this function
    //! can be used to obtain the error message.
